...
from app.services.transaction import mark_transaction_arrived as mark_transaction_arrived_service
...
def _build_transaction(
    seller_id: uuid.UUID,
    *,
    status: TransactionStatus = TransactionStatus.OUT_FOR_DELIVERY,
) -> Transaction:
...
def test_arrival_requires_authentication() -> None:
    client = TestClient(app)
    response = client.post(f"/transactions/{uuid.uuid4()}/arrival")

    assert response.status_code == 401


def test_arrival_rejects_buyer_user(monkeypatch) -> None:
    client = _authenticated_client(monkeypatch, UserRole.BUYER)
    response = client.post(
        f"/transactions/{uuid.uuid4()}/arrival",
...
def test_arrival_allows_transaction_seller(monkeypatch) -> None:
    ...
    def fake_mark_transaction_arrived(_db, transaction_id, seller):
    ...
    monkeypatch.setattr(
        ...
        fake_mark_transaction_arrived,
    )

    client = _authenticated_client(monkeypatch, UserRole.SELLER)
    response = client.post(
        f"/transactions/{transaction.id}/arrival",
        headers=_csrf_headers(),
    )
    ...
    assert body["status"] == "at_door_pending_inspection"


def test_arrival_returns_404_when_transaction_missing(monkeypatch) -> None:
    def fake_mark_transaction_arrived(_db, transaction_id, seller):
...
def test_arrival_returns_403_when_seller_not_owner(monkeypatch) -> None:
    def fake_mark_transaction_arrived(_db, transaction_id, seller):
    ...
    assert response.status_code == 403
    assert response.json()["detail"] == "Only the transaction seller can mark delivery arrival."


def test_arrival_returns_422_for_invalid_prior_state(monkeypatch) -> None:
    def fake_mark_transaction_arrived(_db, transaction_id, seller):
...
def test_service_arrival_moves_to_at_door_pending_inspection() -> None:
    seller = _build_user(UserRole.SELLER)
    transaction = _build_transaction(seller.id, status=TransactionStatus.OUT_FOR_DELIVERY)
    db = _FakeDb(transaction=transaction)

    arrived = mark_transaction_arrived_service(
        cast(Session, db),
        transaction_id=transaction.id,
        seller=seller,
    )

    assert arrived is transaction
    assert arrived.status == TransactionStatus.AT_DOOR_PENDING_INSPECTION
    assert arrived.at_door_at is not None
    ...
    assert all(item.event_type == NotificationEventType.DELIVERY_ARRIVED for item in notifications)


def test_service_arrival_rejects_non_owner_seller() -> None:
    owner = _build_user(UserRole.SELLER)
    other_seller = _build_user(UserRole.SELLER)
    transaction = _build_transaction(owner.id, status=TransactionStatus.OUT_FOR_DELIVERY)
    db = _FakeDb(transaction=transaction)

    with pytest.raises(
        TransactionArrivalForbiddenError,
        match="Only the transaction seller can mark delivery arrival.",
    ):
        mark_transaction_arrived_service(
            cast(Session, db),
            transaction_id=transaction.id,
            seller=other_seller,
        )
...
def test_service_arrival_rejects_invalid_prior_state() -> None:
    seller = _build_user(UserRole.SELLER)
...
def test_service_arrival_rejects_missing_transaction() -> None:
    seller = _build_user(UserRole.SELLER)
    db = _FakeDb(transaction=None)

    with pytest.raises(TransactionNotFoundForArrivalError, match="Transaction not found."):
        mark_transaction_arrived_service(
            cast(Session, db),
